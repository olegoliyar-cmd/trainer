...
Розгортання з нуля, перевірки, деплой, що робити коли червоне —
**[docs/RUNBOOK.md](docs/RUNBOOK.md)**. Написано для людини, не для машини.
...
```bash
node scripts/health_check.mjs      # 56 перевірок: чи все на місці
node scripts/e2e_program_flow.mjs  # 19 перевірок: чи працює сам продукт
```

Перша — «нічого не зламалось». Друга проходить наскрізний сценарій:
тренер складає програму → клієнт бачить її в застосунку → виконує
тренування → тренер бачить виконання. Обидві тільки читають; E2E робить
резервну копію mock-бази й повертає її побайтово.
...
| **Секрети** | ключі відомих форматів + паролі за контекстом · чи `.dev-secrets.env` під забороною |
| **Запускові шляхи** | усе, на що вказує `.claude/launch.json`, існує |
| **Архів на диску** | `archive/` не в git, тож його ніхто не стереже — лінійка рахує файли й байти |
...
- `archive/` (431 МБ) **не входить у git** — там чужі credentials GrekFit,
  GitHub блокував пуш. Його єдина копія — на диску. Див. `archive/README.md`.
...
## Резервна копія і сервер

**GitHub:** `github.com/olegoliyar-cmd/trainer` — гілки `main`, `tilapia`,
мітка `before-cleanup-2026-08-15`.

**Дзеркало на сервері:** Hetzner `grekfit` (178.104.84.6), тека `/opt/trainer`.

```bash
ssh -i ~/.ssh/openclaw_hetzner root@178.104.84.6
trainer-update                          # підтягнути вручну
systemctl status trainer-update.timer   # стан автооновлення (кожні 15 хв)
```

Це **дзеркало, а не друга робоча копія**: ключ доступу до GitHub — лише на
читання, з сервера нічого не запушиш. Секретів (`.env`) там немає свідомо,
тому деплої звідти не робляться.

Шлях правок один: **ноут → GitHub → сервер**.
